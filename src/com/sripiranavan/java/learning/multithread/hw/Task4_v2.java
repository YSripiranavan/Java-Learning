...
public class Task4_v2 {
	...
	private static class SearchEngine {
		...
		public SearchEngine(int[][] matrix) {
			this.matrix = matrix;
			maximumNumbersInEachRow = new ArrayList<Integer>();
		...
		public int getMaxElementInMatrixMultiThreading() throws InterruptedException {
		...
		private int findMaxOfAllThreads(List<Integer> maxInts) {
			return maxInts.stream().mapToInt(i -> i).max().getAsInt();
		}
